    ...
    public:
        ...
        transfer(T begin, T end)
                : data_(const_cast<uint8_t*>(std::to_address(begin))),
                  size_(static_cast<size_type>(reinterpret_cast<const uint8_t*>(std::to_address(end)) - data_))
        {}
        template <typename T>
        ...
        operator std::span<const uint8_t>() const { return { const_cast<const uint8_t*>(data()), size() }; }
