...
namespace usb::df::cdc
{
    ...
    protected:
        ...
        standard::descriptor::interface* get_base_functional_descriptors(class_info cinfo,
    ...
    config::elements<4> config(function& fn, const config::endpoint& out_ep, const config::endpoint& in_ep);

    // active notification endpoint
    config::elements<5> config(function& fn, const config::endpoint& out_ep, const config::endpoint& in_ep,
        const config::endpoint& notify_in_ep);

    config::elements<5> config(function& fn, usb::speed speed, 
        endpoint::address out_ep_addr, endpoint::address in_ep_addr,
