...
namespace usb::df::microsoft
{
    ...
    public:
        ...
        /// @brief  Use a custom non-HID protocol code, as this report layout cannot be made compatible with
        ///         HID report protocol (due to using report ID 0)
        constexpr static inline auto PROTOCOL = static_cast<::hid::protocol>('X');

    private:
        void describe_config(const config::interface& iface, uint8_t if_index, df::buffer& buffer) override;

        const char* ms_compatible_id() const override
        {
