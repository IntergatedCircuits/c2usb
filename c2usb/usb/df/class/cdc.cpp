...
{
    return send_ep(ep_notify_handle(),
                   std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(&data),
                                            sizeof(notification::header) + data.wLength));
}

df::config::elements<4> config(function& fn, const config::endpoint& out_ep,
...
df::config::elements<5> config(function& fn, const config::endpoint& out_ep,
...
df::config::elements<5> config(function& fn, usb::speed speed, endpoint::address out_ep_addr,
                               ...
                               uint8_t notify_in_ep_interval)
{
    return config(fn, config::endpoint::bulk(out_ep_addr, speed),
                  ...
                  config::endpoint::interrupt(notify_in_ep_addr, sizeof(notification::header),
