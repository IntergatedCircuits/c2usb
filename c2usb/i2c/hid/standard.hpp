...
namespace i2c::hid
{
    ...
    public:
        ...
        constexpr hid::opcode opcode() const
        {
            return static_cast<hid::opcode>(buffer_[1]);
        }
        constexpr bool is_extended() const
        {
            return (opcode() >= hid::opcode::GET_REPORT) and
                    (opcode() <= hid::opcode::SET_IDLE) and
    ...
    public:
        ...
        constexpr command(hid::opcode opcode, ::hid::report::type type)
                : command_view(opcode,
                  (static_cast<uint8_t>(type) << 4) | std::min(REPORT_ID, SHORT_REPORT_ID_LIMIT))
        {
