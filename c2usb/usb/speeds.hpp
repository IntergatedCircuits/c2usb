...
namespace usb
{
    ...
    public:
        ...
        constexpr size_t count()           const { return 1u + static_cast<numeric_t>(max) - static_cast<numeric_t>(min); }
        constexpr size_t offset(speed s)   const { return static_cast<numeric_t>(s) - static_cast<numeric_t>(min); }
        constexpr speed at(size_t index)   const { return static_cast<speed>(static_cast<numeric_t>(min) + index); }
    };
}
