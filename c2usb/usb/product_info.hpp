...
struct product_info
{
    ...
    constexpr product_info(uint16_t vend_id, const char_t* vend_name, uint16_t pr_id,
        ...
        : vendor_name(vend_name),
          ...
          product_id(pr_id),
          product_version(pr_ver),
          _serial_number_signed_size(int16_t(-(serial_number.size()))),
          _serial_number(static_cast<const void*>(serial_number.data()))
    {}

    [[nodiscard]] constexpr bool has_serial_number() const
    {
        return _serial_number_signed_size != 0;
    }
    [[nodiscard]] constexpr std::variant<std::string_view, std::span<const uint8_t>>
    serial_number() const
    {
