...
void app_base_function::start(const config::interface& iface, ::hid::protocol prot)
{
    ...
    // start application
    bool success = app_.setup<app_base_function,
            &app_base_function::send_report,
...
{
    if ((get_report_.type() == type) and ((get_report_.id() == 0) or (get_report_.id() == data.front())))
    {
...
void function::describe_config(const config::interface& iface, uint8_t if_index, df::buffer& buffer)
{
    ...
    assert((iface.endpoints()[0].address().direction() == direction::IN) and
            ((1 == iface_desc->bNumEndpoints) or ((2 == iface_desc->bNumEndpoints) and
...
void function::control_setup_request(message& msg, const config::interface& iface)
{
    ...
    switch (msg.request())
    {
        ...
        case GET_REPORT:
        {
            ...
            get_report_ = rs;
            app_.get_report(get_report_, std::span<uint8_t>(msg.buffer().begin(), msg.buffer().max_size()));
        ...
        case SET_IDLE:
            return msg.set_reply(app_.set_idle(
                msg.request().wValue.high_byte() * 4, // conversion to ms
                value_lb));
...
void function::control_data_complete(message& msg, const config::interface& iface)
{
    ...
    switch (msg.request())
    {
        ...
        case GET_DESCRIPTOR:
            if (static_cast<hid::descriptor::type>(msg.request().wValue.high_byte()) == hid::descriptor::type::REPORT)
            {
