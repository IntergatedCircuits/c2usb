...
namespace hid
{
    ...
    private:
        constexpr static auto OFFSET = static_cast<uint8_t>(std::min(report::type::FEATURE, report::type::OUTPUT));
        static_assert((static_cast<uint8_t>(std::max(report::type::FEATURE, report::type::OUTPUT)) - OFFSET) == 1);
    ...
    public:
        ...
        constexpr const report_protocol& report_info() const
        {
            return report_info_;
        }
    ...
    protected: // internal API to the transport
        ...
        result send_report(const std::span<const uint8_t>& data, report::type type = report::type::INPUT)
        {
            assert(data.size() > 0);
            if (has_transport())
            {
        ...
        {
            return send_report(std::span<const uint8_t>(report->data(), sizeof(*report)), report->type());
        }
