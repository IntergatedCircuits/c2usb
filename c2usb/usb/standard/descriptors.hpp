...
struct class_info
{
    ...
    friend standard::descriptor::interface* operator<<(standard::descriptor::interface* desc,
                                                       const class_info& ci);
    friend standard::descriptor::interface_association*
...
enum class type : uint8_t
{
    ...
    CONTAINER_ID = 0x04, /// Defines the instance unique ID used to identify the instance across all
                         /// operating modes
    PLATFORM =
        0x05, /// Defines a device capability specific to a particular platform/operating system
    ...
    SUPERSPEED_PLUS =
        0x0A, /// Defines the set of SuperSpeed Plus USB specific device level capabilities
    PRECISION_TIME_MEASUREMENT = 0x0B, /// Precision Time Measurement (PTM) Capability Descriptor
    WIRELESS_USB_EXT =
        0x0C,         /// Defines the set of Wireless USB 1.1-specific device level capabilities
    ...
    CONFIGURATION_SUMMARY =
        0x10, /// Summarizes configuration information for a function implemented by the device
...
struct configuration_summary
    : public device_capability::descriptor<configuration_summary<CONFIGS_SIZE>>
{
