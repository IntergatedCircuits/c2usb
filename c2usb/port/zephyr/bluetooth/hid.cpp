                                   ...
                                   uint16_t len, uint16_t offset)
{
    ...
    auto protocol = this_->app_.get_protocol();
                            ...
                            uint16_t len, uint16_t offset, gatt::write_flags flag)
{
    ...
    if ((offset > 0) or ((offset + len) > this_->app_.report_info().max_report_size(sel.type())))
    {
        return BT_GATT_ERR(BT_ATT_ERR_INVALID_OFFSET);
    }
...
::hid::result service::send_report(const std::span<const uint8_t>& data, report::type type)
{
    ...
    const gatt::attribute* attr = input_report_attr(report::id(data.front()));
    if (attr == nullptr)
    {
        return result::INVALID;
    }
                                                   ...
                                                   gatt::ccc_store* ccc, ::hid::report::id::type id)
{
    ...
    *ccc = gatt::ccc_store(
        ...
        [](::bt_conn* conn, const gatt::attribute* attr, gatt::ccc_flags flags) -> ssize_t
        {
            attr -= 2; // distance between characteristic value and ccc descriptor
            auto* this_ = reinterpret_cast<service*>(attr->user_data);

            LOG_DBG("report CCC set: %u", static_cast<uint16_t>(flags));
            if (flags != gatt::ccc_flags::NONE)
            {
                return this_->start_app(conn) ? sizeof(flags)
...
bool service::start_app(::bt_conn* conn, ::hid::protocol protocol)
{
    ...
    {
        return expected == conn;
    }

    auto result = app_.setup(this, protocol);
    LOG_INF("starting HID app %u", result);
    if (!result)
    {
                                                    ...
                                                    const std::span<gatt::ccc_store>& cccs, flags f)
{
    ...
    // TODO add support for boot attributes
